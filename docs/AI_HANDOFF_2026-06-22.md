...
5. **Branch is pushed and in sync with `origin/ui-review-polish` (done 2026-06-22, tip `7f35081`).**
   The final self-review is done — see `docs/PRE_PUSH_REVIEW_2026-06-22.md` (approved, no blockers;
   3 minor follow-ups). NOTE: this repo's **default branch is `ui-review-polish` itself** and `gh`
   isn't installed, so there is no separate `main` to open a PR against — pushing the branch *is*
   the publish step here. If a PR review is wanted later, first designate/create a base branch.
   Remaining work: validate `docs/telemetry-queries.sql` against real rows once events flow, plus
   the deploy-time actions in §5 / §10 #1 (apply `0006`+`0007`, set `ADMIN_EMAILS`, toggle
   leaked-password protection). **Never force-push or rewrite the shared branch's history.**
